# 🧠 Memorize Game – Stanford CS193p (2023)

A SwiftUI-based memory game built as part of the **Stanford University** course [CS193p: Developing Applications for iOS](https://cs193p.stanford.edu/2023).  
This project demonstrates concepts such as MVVM architecture, animation, state management, and declarative UI programming in SwiftUI.

---

## 🎓 About the Course

![Stanford-University-Logo](https://github.com/user-attachments/assets/f4991885-503c-46dd-8897-1d4760c5cb87)

📚 [CS193p: Developing Applications for iOS using SwiftUI](https://cs193p.stanford.edu/2023)

---

## 🚀 Technologies Used

[![Swift](https://img.shields.io/badge/Swift-FA7343?logo=swift&logoColor=white)](https://developer.apple.com/swift/)  
[![SwiftUI](https://img.shields.io/badge/SwiftUI-0D1117?logo=swift&logoColor=blue)](https://developer.apple.com/xcode/swiftui/)  
[![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)](https://git-scm.com/)  
[![GitHub](https://img.shields.io/badge/GitHub-181717?logo=github&logoColor=white)](https://github.com/danimelenge/MemorizeGame)

---

## 🧩 Features

- ✅ Card flipping and matching logic  
- 🎨 Custom themes and emojis  
- 🔄 Shuffle and restart game  
- 📱 Responsive SwiftUI layout  
- 💡 MVVM architecture pattern  

---

## 📱 Screenshots

<img width="405" alt="Screenshot 1" src="https://github.com/user-attachments/assets/0767b5ae-228a-4a69-bd78-75c0cc3d9115" />
<img width="405" alt="Screenshot 2" src="https://github.com/user-attachments/assets/3636fb96-da50-4039-b9f5-46b35a0d5f09" />
<img width="405" alt="Screenshot 3" src="https://github.com/user-attachments/assets/c7c342a3-bd00-40b7-a39a-b5f55e0a27bd" />
<img width="1179" height="2556" alt="Simulator Screenshot - iPhone 15 Pro - 2025-11-03 at 15 13 54" src="https://github.com/user-attachments/assets/6db24c1c-9645-41bb-bf9d-86a4df11b94f" />

---

## 📚 Resources

- [📘 Swift Documentation](https://developer.apple.com/documentation/swift)  
- [📗 SwiftUI Documentation](https://developer.apple.com/documentation/swiftui)  
- [📙 Stanford CS193p (2023)](https://cs193p.stanford.edu/2023)

---

## 📄 License

This project is for educational purposes as part of Stanford University’s CS193p course.  
All rights belong to their respective creators and institutions.

---

## 🔗 Repository

[👉 View on GitHub](https://github.com/danimelenge/MemorizeGame)
